...
$localZipMaster = "C:\Users\mahin khan\.gemini\antigravity\scratch\portfolio-showcase\IINSHA_AUTONOMOUS_GROWTH_AND_FREE_VPS_SUITE.zip"

Compress-Archive -Path "$docsDir\*" -DestinationPath $localZipMaster -Force
Copy-Item -Path $localZipMaster -Destination "$targetDir\IINSHA_AUTONOMOUS_GROWTH_AND_FREE_VPS_SUITE.zip" -Force
