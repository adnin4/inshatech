...
$localZipFoundation = "C:\Users\mahin khan\.gemini\antigravity\scratch\portfolio-showcase\IINSHA_FOUNDATION_GATE_LOCKED.zip"

Compress-Archive -Path "$docsDir\*" -DestinationPath $localZipFoundation -Force
Copy-Item -Path $localZipFoundation -Destination "$targetDir\IINSHA_FOUNDATION_GATE_LOCKED.zip" -Force
