...
// 1. docs/REALITY_MATRIX.md
const realityMatrix = `# 🛡️ IINSHA AI-BOS — REALITY MATRIX & TECHNICAL GROUND TRUTH (v2026.08)
...
fs.writeFileSync("docs/REALITY_MATRIX.md", realityMatrix, "utf8");

// 2. Clean up ai_brain/tool_executor.js to remove fake company fixtures
const toolExecutorPath = "ai_brain/tool_executor.js";
let toolExecutorContent = fs.readFileSync(toolExecutorPath, "utf8");
toolExecutorContent = toolExecutorContent.replace(
    /companies:\s*\[[\s\S]*?ApexFlow Analytics[\s\S]*?\]/g,
    `companies: [], note: 'Connect Supabase database or Playwright scraper to stream live company records.'`
);
fs.writeFileSync(toolExecutorPath, toolExecutorContent, "utf8");

console.log("docs/REALITY_MATRIX.md created and ai_brain/tool_executor.js cleaned successfully!");
