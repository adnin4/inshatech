 ...
 *  - 4-Tier Gamified Progression Ladder (Bronze Ã¢Å¾ Silver Ã¢Å¾ Gold Ã¢Å¾ Legend) with Milestone Rewards
...
(function() {
    ...
    const OFFERS = [
        { id: 'b2b-lead-swarm', name: 'B2B SaaS 5-Agent Hunter Swarm', priceUSD: 850, priceBDT: 104125, commUSD: 212.50, commBDT: 26031, epc: '$8.40', conversion: '3.4%', category: 'Lead Gen', tag: 'Ã°Å¸¥ Top Converting', target: 'store.html' },
        { id: 'ecommerce-ai-whatsapp', name: '24/7 E-Commerce WhatsApp Sales Agent', priceUSD: 750, priceBDT: 91875, commUSD: 187.50, commBDT: 22968, epc: '$7.80', conversion: '4.2%', category: 'E-Commerce', tag: 'Ã¢Å¡Â¡ 20-Min Setup', target: 'store.html' },
        { id: 'voice-ai-receptionist', name: 'AI Voice Receptionist (Twilio + WebRTC)', priceUSD: 1800, priceBDT: 220500, commUSD: 450.00, commBDT: 55125, epc: '$12.20', conversion: '2.1%', category: 'Voice AI', tag: 'Ã°Å¸Å½¯Â¸Â High Ticket', target: 'store.html' },
        { id: 'n8n-docker-cluster', name: 'Self-Hosted n8n VPS Cluster Deployment', priceUSD: 497, priceBDT: 60882, commUSD: 124.25, commBDT: 15220, epc: '$6.50', conversion: '5.8%', category: 'Infra', tag: 'Ã°Å¸° 90% Cost Saving', target: 'store.html' },
        { id: 'invoice-ocr-pipeline', name: 'Autonomous Invoice & Document OCR Pipeline', priceUSD: 249, priceBDT: 30502, commUSD: 62.25, commBDT: 7625, epc: '$4.90', conversion: '6.4%', category: 'OCR', tag: 'Ã¢Å¡Â¡ Fast Turnaround', target: 'store.html' }
    ];
    ...
    const MARKETING_VAULT = [
        {
            id: 'mv-li-01',
            channel: 'Ã°Å¸± LinkedIn / X Viral Post',
        ...
        {
            id: 'mv-email-01',
            channel: 'Ã¢Å“¯Â¸Â CEO / Founder Cold Email Hook',
        ...
        {
            id: 'mv-wa-bn-01',
            channel: 'Ã°Å¸¬ WhatsApp / Messenger Pitch (Bangla)',
            title: 'F-Commerce & E-Commerce Store Pitch (Bangla)',
            category: 'messaging',
            text: `Ã Â¦ Ã Â¦Â¸Ã Â¦Â¸Ã Â¦Â¾Ã Â¦Â²Ã Â¦Â¾Ã Â¦Â®Ã Â§Â Ã Â¦ Ã Â¦Â²Ã Â¦Â¾Ã Â¦ Â¦•Ã Â§ÂÃ Â¦Â® {{Name}} Ã Â¦Â­Ã Â¦Â¾Ã Â¦\n\nÃ Â¦ Ã Â¦ÂªÃ Â¦Â¨Ã Â¦Â¾Ã Â¦Â° Ã Â¦ Â¦Â¨Ã Â¦Â²Ã Â¦Â¾Ã Â¦ Â¦Â¨ Ã Â¦Â¸Ã Â§ÂÃ Â¦Å¸Ã Â§ Â¦Â°Ã Â§ Ã Â¦•Ã Â¦Â¾Ã Â¦Â¸Ã Â§ÂÃ Â¦Å¸Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â°Ã Â¦Â¦Ã Â§ Â¦Â° Ã Â¦ Â¦Â¨Ã Â¦Â¬Ã Â¦•Ã Â§ÂÃ Â¦Â¸ Ã Â¦Â°Ã Â¦Â¿Ã Â¦ÂªÃ Â§ÂÃ Â¦Â²Ã Â¦Â¾Ã Â¦ Ã Â¦Â¡Ã Â§ Â¦Â²Ã Â¦Â¿Ã Â¦Â­Ã Â¦Â¾Ã Â¦Â°Ã Â¦Â¿ Ã Â¦Å¡Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Å“ Ã Â¦•Ã Â§ÂÃ Â¦Â¯Ã Â¦Â¾Ã Â¦Â²Ã Â¦•Ã Â§ÂÃ Â¦Â²Ã Â§ Â¦Â¶Ã Â¦Â¨ Ã Â¦ÂÃ Â¦Â¬Ã Â¦ Ã Â¦ Â¦Â°Ã Â§ÂÃ Â¦Â¡Ã Â¦Â¾Ã Â¦Â° Ã Â¦•Ã Â¦Â¨Ã Â¦Â«Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Â® Ã Â¦•Ã Â¦Â°Ã Â¦Â¤Ã Â§ Ã Â¦•Ã Â¦Â¿ Ã Â¦Â°Ã Â¦Â¾Ã Â¦Â¤Ã Â§ Â¦ Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â¨Ã Â§ÂÃ Â¦Â· Ã Â¦Â¬Ã Â¦Â¸Ã Â¦Â¿Ã Â§Å¸Ã Â§ Ã Â¦Â°Ã Â¦Â¾Ã Â¦–Ã Â¦Â¤Ã Â§ Ã Â¦Â¹Ã Â¦Å¡Ã Â§ÂÃ Â¦ Â§\n\nIINSHA-Ã Â¦ÂÃ Â¦Â° 24/7 AI WhatsApp & Messenger Sales Bot Ã Â¦ Ã Â¦ÂªÃ Â¦Â¨Ã Â¦Â¾Ã Â¦Â° Ã Â¦ Â§Å¸Ã Â§ Â¦Â¬Ã Â¦Â¸Ã Â¦Â¾Ã Â¦ Â¦Å¸Ã Â§ Â¦Â° Ã Â¦ÂªÃ Â§ÂÃ Â¦Â°Ã Â§ Ã Â¦•Ã Â§ÂÃ Â¦Â¯Ã Â¦Â¾Ã Â¦Å¸Ã Â¦Â¾Ã Â¦Â²Ã Â¦— Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â¤Ã Â§ÂÃ Â¦Â° Ã Â§Â¨Ã Â§Â¦ Ã Â¦Â®Ã Â¦Â¿Ã Â¦Â¨Ã Â¦Â¿Ã Â¦Å¸Ã Â§ Ã Â¦ÂªÃ Â§Å“Ã Â§ Ã Â¦Â¨Ã Â¦Â¿Ã Â§Å¸Ã Â§ Ã Â¦•Ã Â¦Â¾Ã Â¦Â¸Ã Â§ÂÃ Â¦Å¸Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â°Ã Â§ Â¦Â° Ã Â¦Â¸Ã Â¦Â¾Ã Â¦Â¥Ã Â§ Ã Â¦Â¬Ã Â¦Â¾Ã Â¦ Â¦Â²Ã Â¦Â¾, Ã Â¦Â¬Ã Â¦Â¾Ã Â¦ Â¦Â²Ã Â¦Â¿Ã Â¦Â¶ Ã Â¦ Ã Â¦ Â¦ Â¦Â²Ã Â¦Â¿Ã Â¦Â¶Ã Â§ Ã Â¦•Ã Â¦Â¥Ã Â¦Â¾ Ã Â¦Â¬Ã Â¦Â²Ã Â§ Ã Â¦Â¸Ã Â§ÂÃ Â¦Â¬Ã Â§Å¸Ã Â¦ Â¦•Ã Â§ÂÃ Â¦Â°Ã Â¦Â¿Ã Â§Å¸Ã Â¦Â­Ã Â¦Â¾Ã Â¦Â¬Ã Â§ Ã Â¦ Â¦Â°Ã Â§ÂÃ Â¦Â¡Ã Â¦Â¾Ã Â¦Â° Ã Â¦•Ã Â¦Â¨Ã Â¦Â«Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Â® Ã Â¦•Ã Â¦Â°Ã Â§ Â¥Â¤\n\nÃ Â¦Â¬Ã Â¦Â¿Ã Â¦Â¸Ã Â§ÂÃ Â¦Â¤Ã Â¦Â¾Ã Â¦Â°Ã Â¦Â¿Ã Â¦Â¤ Ã Â¦Â¦Ã Â§ Â¦–Ã Â§ÂÃ Â¦Â¨ Ã Â¦ÂÃ Â¦Â¬Ã Â¦ Ã Â¦ Ã Â¦ÂªÃ Â¦Â¨Ã Â¦Â¾Ã Â¦Â° Ã Â¦Â¬Ã Â§ÂÃ Â¦Â¯Ã Â¦Â¬Ã Â¦Â¸Ã Â¦Â¾Ã Â¦Â° Ã Â¦Å“Ã Â¦Â¨Ã Â§ÂÃ Â¦Â¯ Ã Â¦Â«Ã Â§ÂÃ Â¦Â°Ã Â¦Â¿ Ã Â¦Â¡Ã Â§ Â¦Â®Ã Â§ Ã Â¦Â¬Ã Â§ÂÃ Â¦• Ã Â¦•Ã Â¦Â°Ã Â§ÂÃ Â¦Â¨:\n{{REF_LINK}}`
        },
        {
            id: 'mv-wa-en-01',
            channel: 'Ã°Å¸¬ WhatsApp / SMS Quick DM (English)',
        ...
        root.innerHTML = `
            <div id="affiliate-portal-wrapper" style="display:flex; flex-direction:column; gap:24px;">
                ...
                <div class="ipc-card ipc-card-glowing" style="background:rgba(15,23,42,0.95); border:1px solid var(--accent-cyan); border-radius:18px; padding:24px; box-shadow:0 20px 50px rgba(0,0,0,0.8);">
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:14px; margin-top:20px;">
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(16,185,129,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸° Unpaid Balance (Ready)</span>
                            ...
                            <span style="font-size:0.72rem; color:#38bdf8;">Ã¢† Ã Â§Â³${Math.round(partner.unpaid * 122.50).toLocaleString()} BDT</span>
                        </div>
                        ...
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(245,158,11,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã¢ÂÂ³ Pending Clearance</span>
                        ...
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(0,242,254,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸½ Lifetime Disbursed</span>
                        ...
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(168,85,247,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸† Average EPC (Yield)</span>
                ...
                <div style="display:flex; gap:8px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:8px; overflow-x:auto; -webkit-overflow-scrolling:touch;">
                    <button onclick="window.switchAffiliateTab('dashboard')" class="lab-tab active" id="tab-btn-dashboard" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸  Live Cockpit</button>
                    ...
                    <button onclick="window.switchAffiliateTab('links')" class="lab-tab" id="tab-btn-links" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸ Link & SubID Studio</button>
                    ...
                    <button onclick="window.switchAffiliateTab('milestones')" class="lab-tab" id="tab-btn-milestones" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸Â  Tiers & Rewards</button>
                    <button onclick="window.switchAffiliateTab('payouts')" class="lab-tab" id="tab-btn-payouts" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸³ Payout Ledger</button>
                    <button onclick="window.switchAffiliateTab('academy')" class="lab-tab" id="tab-btn-academy" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸¡ Growth Academy</button>
                </div>
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(300px, 1fr)); gap:20px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0;">
                            <h3 style="color:#fff; font-size:1.05rem; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                                <span>Ã°Å¸°Ã¯Â¸Â</span> Real-Time Traffic Telemetry
                            </h3>
                            ...
                            <div style="margin-top:14px; padding:10px; background:rgba(52,211,153,0.1); border:1px solid rgba(52,211,153,0.3); border-radius:8px; font-size:0.78rem; color:#a7f3d0; font-family:var(--font-mono);">
                                Ã°Å¸¡Ã¯Â¸Â Server-Side Fraud Filter: <strong>${partner.trafficQuality}</strong>
                            </div>
                        </div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0;">
                            <h3 style="color:#fff; font-size:1.05rem; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                                <span>Ã°Å¸²</span> 1-Click Social Broadcast Station
                            </h3>
                    ...
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px; border-color:var(--accent-cyan);">
                        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:18px;">
                            <div>
                                ...
                                <h3 style="color:#fff; font-size:1.3rem; margin:4px 0 0 0;">Ã¢Å¡Â¡ Custom AI Marketing Pitch Generator</h3>
                        ...
                        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:14px; margin-bottom:16px;">
                            ...
                            <div>
                                ...
                                <select id="ai-work-niche" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                    <option value="ecom">Ã°Å¸€™ E-Commerce & F-Commerce Store</option>
                                    <option value="saas">Ã°Å¸» B2B SaaS & Tech Agency</option>
                                    <option value="realestate">Ã°Å¸ÂÂ¢ Real Estate & Property Developers</option>
                                    <option value="clinic">Ã°Å¸ÂÂ¥ Clinics, Dental & Healthcare</option>
                                    <option value="general">Ã¢Å¡Â¡ Generic High-Zapier Spender</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="ai-work-format" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                    <option value="wa_bn">Ã°Å¸¬ WhatsApp Direct Pitch (Bangla)</option>
                                    <option value="wa_en">Ã°Å¸¬ WhatsApp Direct Pitch (English)</option>
                                    <option value="li_post">Ã°Å¸± LinkedIn Viral Post</option>
                                    <option value="cold_email">Ã¢Å“¯Â¸Â B2B Cold Email Sequence</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <button onclick="window.generateCustomMarketingPitch()" class="btn btn-primary" style="width:100%; justify-content:center; padding:12px; font-weight:800; font-size:0.95rem; margin-bottom:16px;">
                            Ã¢Å¡Â¡ Generate Bespoke Marketing Copy & Link Ã¢ 
                        </button>
                        ...
                        <div style="position:relative;">
                            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                                ...
                                <button onclick="window.copyGeneratedPitch()" class="btn btn-glass-sm" style="font-size:0.72rem; padding:4px 10px;">Ã°Å¸€¹ Copy Text</button>
                            </div>
                ...
                <div id="tab-panel-crm" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:18px;">
                            <div>
                                <h3 style="color:#fff; font-size:1.25rem; margin:0;">Ã°Å¸Å½Â¯ Partner Outreach Pipeline CRM</h3>
                                    ...
                                    ${getProspects().map(p => `
                                        <tr style="border-bottom:1px solid rgba(255,255,255,0.04); font-family:var(--font-mono);">
                                            ...
                                            <td style="padding:10px;">
                                                <button onclick="window.sendDirectProspectMessage('${p.id}')" class="btn btn-glass-sm" style="font-size:0.7rem; padding:2px 8px;">Ã°Å¸¬ Message</button>
                                            </td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                ...
                <div id="tab-panel-links" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px;">
                        <h3 style="color:#fff; font-size:1.2rem; margin-bottom:6px;">Ã°Å¸ Advanced Custom Link & SubID Generator</h3>
                        ...
                        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:14px; margin-bottom:18px;">
                            <div>
                                ...
                                <select id="link-studio-target" onchange="window.updateStudioGeneratedLink()" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem;">
                                    <option value="/">Ã°Å¸ÂÂ  Main Homepage (High Intent)</option>
                                    <option value="/store.html">Ã°Å¸€™ Turnkey Service Store ($29 - $1,800)</option>
                                    <option value="/marketplace.html">Ã°Å¸ÂÂ¬ AI Multi-Agent Marketplace</option>
                                    <option value="/compare.html">Ã¢Å¡¯Â¸Â Comparison & ROI Matrix</option>
                                </select>
                            </div>
                        ...
                        <div style="background:rgba(0,0,0,0.6); border:1px solid var(--accent-cyan); border-radius:10px; padding:14px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                            ...
                            <div style="display:flex; gap:8px;">
                                <button onclick="window.copyStudioLink()" class="btn btn-primary-sm" style="font-weight:700;">Ã°Å¸€¹ Copy Link</button>
                                <button onclick="window.toggleQrCodeModal()" class="btn btn-glass-sm">Ã°Å¸± QR Code</button>
                            </div>
                        </div>

                        <!-- Deep Link Offer Cards -->
                        <div style="margin-top:28px;">
                            <h4 style="color:#fff; font-size:0.95rem; margin-bottom:12px;">Ã°Å¸¦ Product-Specific Direct Deep Links:</h4>
                            <div style="display:flex; flex-direction:column; gap:8px;">
                                ${OFFERS.map(o => `
                                    <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:8px; padding:10px 14px; flex-wrap:wrap; gap:8px;">
                                        ...
                                        <button onclick="window.copyDeepLinkDirect('${o.id}', '${partner.refCode}')" class="btn btn-glass-sm" style="font-size:0.72rem; padding:4px 10px;">Ã°Å¸€¹ Copy Product Link</button>
                                    </div>
                                `).join('')}
                            </div>
                        </div>
                ...
                <div id="tab-panel-vault" class="aff-tab-panel" style="display:none;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
                        <div>
                            <h3 style="color:#fff; font-size:1.2rem; margin:0;">Ã°Å¸Å½Â¨ High-Converting Marketing Asset Center</h3>
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(300px, 1fr)); gap:16px;" id="vault-assets-grid">
                        ${MARKETING_VAULT.map(a => {
                            const populated = a.text.replace(/\{\{REF_LINK\}\}/g, partnerRefLink).replace(/\{\{PartnerName\}\}/g, partner.name).replace(/\{\{CompanyName\}\}/g, 'Prospect Inc').replace(/\{\{FirstName\}\}/g, 'Founder').replace(/\{\{Name\}\}/g, 'Ã Â¦Â­Ã Â¦Â¾Ã Â¦');
                            return `
                                <div class="ipc-card vault-item" data-cat="${a.category}" style="background:rgba(15,23,42,0.85); margin:0; padding:18px;">
                                    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;">
                                        ...
                                        <button onclick="window.copyAssetText('${a.id}')" class="btn btn-primary-sm" style="padding:4px 10px; font-size:0.72rem;">Ã°Å¸€¹ Copy</button>
                                    </div>
                ...
                <div id="tab-panel-coach" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px; border-color:var(--accent-cyan);">
                        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:16px;">
                            ...
                            <button onclick="window.generateCustomNicheStrategy()" class="btn btn-primary-sm">Ã¢Å“Â¨ Generate Custom Plan</button>
                        </div>
                        ...
                        <div id="ai-coach-custom-box" style="background:rgba(0,0,0,0.6); border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:16px; font-size:0.82rem; color:#cbd5e1; line-height:1.6;">
                            <strong style="color:var(--accent-cyan); display:block; margin-bottom:4px;">Ã°Å¸¡ AI Partner Recommendation:</strong>
                ...
                <div id="tab-panel-milestones" class="aff-tab-panel" style="display:none;">
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:rgba(255,255,255,0.1); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸Å’Â± Starter Bronze</div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:var(--accent-cyan); box-shadow:0 0 20px rgba(0,242,254,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸¥ VIP Silver</div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:var(--accent-gold); box-shadow:0 0 20px rgba(245,158,11,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸€˜ Elite Gold (Active)</div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:#ec4899; box-shadow:0 0 20px rgba(236,72,153,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸½ Legend Diamond</div>
                ...
                <div id="tab-panel-payouts" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:22px; border-radius:16px; margin:0;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
                            <h3 style="color:#fff; font-size:1.15rem; margin:0;">Ã°Å¸³ Canonical Transaction & Payout Ledger</h3>
                            <button onclick="window.requestPartnerWithdrawal()" class="btn btn-primary-sm">Ã¢Å¡Â¡ Request Instant Payout</button>
                        </div>
                ...
                <div id="tab-panel-academy" class="aff-tab-panel" style="display:none;">
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:16px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0; padding:20px;">
                            <span style="font-size:1.8rem;">Ã°Å¸Å½</span>
                            ...
                            <button onclick="alert('Ã°Å¸œ Module 1 Guide:\n\n1. Search founders hiring for Operations/Data entry on LinkedIn.\n2. Send Swipe #1.\n3. When they reply, drop your affiliate link to book the free video teardown.')" class="btn btn-glass-sm" style="width:100%; margin-top:10px;">Read Playbook Ã¢ </button>
                        </div>

                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0; padding:20px;">
                            <span style="font-size:1.8rem;">Ã°Å¸¬</span>
                            ...
                            <button onclick="alert('Ã°Å¸œ Module 2 Guide:\n\n1. Target e-commerce brands with 50+ daily orders.\n2. Send Bangla WhatsApp swipe.\n3. Show how the bot answers in 3 seconds & confirms cash-on-delivery orders.')" class="btn btn-glass-sm" style="width:100%; margin-top:10px;">Read Playbook Ã¢ </button>
                        </div>

                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0; padding:20px;">
                            <span style="font-size:1.8rem;">Ã¢Å¡¯Â¸Â</span>
                            ...
                            <button onclick="alert('Ã°Å¸œ Module 3 Guide:\n\n1. Highlight 90% cost savings.\n2. Emphasize self-hosted data privacy & PostgreSQL backups on $5.99/mo VPS.')" class="btn btn-glass-sm" style="width:100%; margin-top:10px;">Read Playbook Ã¢ </button>
                        </div>
        ...
        root.innerHTML = `
            <div id="affiliate-auth-gateway" class="ipc-card ipc-card-glowing" style="background:rgba(15,23,42,0.95); border:1px solid var(--accent-cyan); border-radius:18px; padding:32px; max-width:850px; margin:0 auto; box-shadow:0 25px 60px rgba(0,0,0,0.85);">
                ...
                <div style="display:flex; justify-content:center; gap:10px; margin-bottom:24px;">
                    <button onclick="window.switchAuthMode('signin')" id="auth-tab-signin" class="lab-tab active" style="padding:10px 24px; font-size:0.9rem; font-weight:800; border-radius:10px;">Ã°Å¸€˜ Partner Sign In</button>
                    <button onclick="window.switchAuthMode('register')" id="auth-tab-register" class="lab-tab" style="padding:10px 24px; font-size:0.9rem; font-weight:800; border-radius:10px;">Ã°Å¸Å¡ Register New Account</button>
                </div>
                    ...
                    <form onsubmit="window.handlePartnerRegister(event)" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                        ...
                        <div>
                            ...
                            <select id="reg-payout-method" required style="width:100%; background:rgba(15,23,42,0.9); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px 14px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                <option value="bKash (Personal / Merchant)">Ã°Å¸§Ã°Å¸© bKash (Personal / Merchant)</option>
                                <option value="Nagad">Ã°Å¸§Ã°Å¸© Nagad</option>
                                <option value="Wise Bank Wire (USD/GBP)">Ã°Å¸Å’Â Wise Bank Wire (USD/GBP/EUR)</option>
                                <option value="Local Bank Transfer">Ã°Å¸ÂÂ¦ Local Bank Transfer</option>
                                <option value="USDT Crypto (TRC20)">Ã¢Å¡Â¡ USDT Crypto (TRC20)</option>
                            </select>
                        </div>
                        ...
                        <div style="grid-column: 1 / -1; margin-top:8px;">
                            <button type="submit" class="btn btn-primary" style="width:100%; background:linear-gradient(135deg, #10b981, #059669); padding:14px; font-size:1rem; font-weight:800; border-radius:10px; cursor:pointer; justify-content:center;">
                                Ã°Å¸Å¡ Create Account & Launch Dashboard Ã¢ 
                            </button>
                        </div>
                    </form>
                </div>
    ...
    window.handlePartnerSignIn = function(e) {
        ...
        if (matchedPartner) {
            ...
            window.showAffiliateToast(`Ã°Å¸€¹ Welcome back, ${matchedPartner.name}!`);
        } else {
            ...
            const newP = {
                ...
                payoutAccount: '01629286887',
                refCode: newId,
                tier: 'Ã°Å¸Å’Â± Starter Bronze (15%)',
                tierLevel: 1,
                unpaid: 0.00,
                pending: 0.00,
                paidOut: 0.00,
            ...
            window.initAffiliatePortal();
            window.showAffiliateToast(`Ã°Å¸Å½ Welcome to IINSHA Partner OS, ${newP.name}!`);
        }
    };
    ...
    window.handlePartnerRegister = function(e) {
        ...
        setActivePartner(newId);
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã°Å¸Å½ Account Created! Welcome, ${name}! Your link: /?ref=${newId}`);
    };
    ...
    window.quickDemoLogin = function(partnerId) {
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã°Å¸€˜ Switched account to ${partnerId}!`);
    };
    ...
    window.generateCustomMarketingPitch = function() {
        ...
        let text = '';
        if (format === 'wa_bn') {
            text = `Ã Â¦ Ã Â¦Â¸Ã Â¦Â¸Ã Â¦Â¾Ã Â¦Â²Ã Â¦Â¾Ã Â¦Â®Ã Â§Â Ã Â¦ Ã Â¦Â²Ã Â¦Â¾Ã Â¦ Â¦•Ã Â§ÂÃ Â¦Â® ${name} Ã Â¦Â­Ã Â¦Â¾Ã Â¦\n\n${company}-Ã Â¦ÂÃ Â¦Â° Ã Â¦•Ã Â¦Â¾Ã Â¦Â¸Ã Â§ÂÃ Â¦Å¸Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â°Ã Â¦Â¦Ã Â§ Â¦Â° Ã Â¦ Â¦Â¨Ã Â¦Â¬Ã Â¦•Ã Â§ÂÃ Â¦Â¸ Ã Â¦Â°Ã Â¦Â¿Ã Â¦ÂªÃ Â§ÂÃ Â¦Â²Ã Â¦Â¾Ã Â¦ Ã Â¦Â¡Ã Â§ Â¦Â²Ã Â¦Â¿Ã Â¦Â­Ã Â¦Â¾Ã Â¦Â°Ã Â¦Â¿ Ã Â¦Å¡Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Å“ Ã Â¦•Ã Â§ÂÃ Â¦Â¯Ã Â¦Â¾Ã Â¦Â²Ã Â¦•Ã Â§ÂÃ Â¦Â²Ã Â§ Â¦Â¶Ã Â¦Â¨ Ã Â¦ÂÃ Â¦Â¬Ã Â¦ Ã Â¦ Â¦Â°Ã Â§ÂÃ Â¦Â¡Ã Â¦Â¾Ã Â¦Â° Ã Â¦•Ã Â¦Â¨Ã Â¦Â«Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Â® Ã Â¦•Ã Â¦Â°Ã Â¦Â¤Ã Â§ Ã Â¦•Ã Â¦Â¿ Ã Â¦Â°Ã Â¦Â¾Ã Â¦Â¤Ã Â§ Â¦ Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â¨Ã Â§ÂÃ Â¦Â· Ã Â¦Â¬Ã Â¦Â¸Ã Â¦Â¿Ã Â§Å¸Ã Â§ Ã Â¦Â°Ã Â¦Â¾Ã Â¦–Ã Â¦Â¤Ã Â§ Ã Â¦Â¹Ã Â¦Å¡Ã Â§ÂÃ Â¦ Â§\n\nIINSHA-Ã Â¦ÂÃ Â¦Â° 24/7 AI WhatsApp & Messenger Sales Bot Ã Â¦ Ã Â¦ÂªÃ Â¦Â¨Ã Â¦Â¾Ã Â¦Â° Ã Â¦ Â§Å¸Ã Â§ Â¦Â¬Ã Â¦Â¸Ã Â¦Â¾Ã Â¦ Â¦Å¸Ã Â§ Â¦Â° Ã Â¦ÂªÃ Â§ÂÃ Â¦Â°Ã Â§ Ã Â¦•Ã Â§ÂÃ Â¦Â¯Ã Â¦Â¾Ã Â¦Å¸Ã Â¦Â¾Ã Â¦Â²Ã Â¦— Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â¤Ã Â§ÂÃ Â¦Â° Ã Â§Â¨Ã Â§Â¦ Ã Â¦Â®Ã Â¦Â¿Ã Â¦Â¨Ã Â¦Â¿Ã Â¦Å¸Ã Â§ Ã Â¦ÂªÃ Â§Å“Ã Â§ Ã Â¦Â¨Ã Â¦Â¿Ã Â§Å¸Ã Â§ Ã Â¦•Ã Â¦Â¾Ã Â¦Â¸Ã Â§ÂÃ Â¦Å¸Ã Â¦Â®Ã Â¦Â¾Ã Â¦Â°Ã Â§ Â¦Â° Ã Â¦Â¸Ã Â¦Â¾Ã Â¦Â¥Ã Â§ Ã Â¦Â¬Ã Â¦Â¾Ã Â¦ Â¦Â²Ã Â¦Â¾, Ã Â¦Â¬Ã Â¦Â¾Ã Â¦ Â¦Â²Ã Â¦Â¿Ã Â¦Â¶ Ã Â¦ Ã Â¦ Â¦ Â¦Â²Ã Â¦Â¿Ã Â¦Â¶Ã Â§ Ã Â¦•Ã Â¦Â¥Ã Â¦Â¾ Ã Â¦Â¬Ã Â¦Â²Ã Â§ Ã Â¦Â¸Ã Â§ÂÃ Â¦Â¬Ã Â§Å¸Ã Â¦ Â¦•Ã Â§ÂÃ Â¦Â°Ã Â¦Â¿Ã Â§Å¸Ã Â¦Â­Ã Â¦Â¾Ã Â¦Â¬Ã Â§ Ã Â¦ Â¦Â°Ã Â§ÂÃ Â¦Â¡Ã Â¦Â¾Ã Â¦Â° Ã Â¦•Ã Â¦Â¨Ã Â¦Â«Ã Â¦Â¾Ã Â¦Â°Ã Â§ÂÃ Â¦Â® Ã Â¦•Ã Â¦Â°Ã Â§ Â¥Â¤\n\nÃ Â¦Â«Ã Â§ÂÃ Â¦Â°Ã Â¦Â¿ Ã Â¦Â¡Ã Â§ Â¦Â®Ã Â§ Ã Â¦ÂÃ Â¦Â¬Ã Â¦ Ã Â§Â¨Ã Â§Âª Ã Â¦ËœÃ Â¦Â£Ã Â§ÂÃ Â¦Å¸Ã Â¦Â¾Ã Â¦Â° Ã Â¦Â­Ã Â¦Â¿Ã Â¦Â¡Ã Â¦Â¿Ã Â¦ Ã Â¦ Â¦Â¡Ã Â¦Â¿Ã Â¦Å¸ Ã Â¦Â¦Ã Â§ Â¦–Ã Â¦Â¤Ã Â§ Ã Â¦Â¨Ã Â¦Â¿Ã Â¦Å¡Ã Â§ Â¦Â° Ã Â¦Â²Ã Â¦Â¿Ã Â¦ Â¦•Ã Â§ Ã Â¦Â¯Ã Â¦Â¾Ã Â¦Â¨:\n${partnerRefLink}&subid=wa_pitch`;
        ...
        window.showAffiliateToast('Ã¢Å“Â¨ Custom marketing pitch generated with your affiliate link embedded!');
    };
    ...
    window.copyGeneratedPitch = function() {
        ...
        if (outputEl) {
            ...
            window.showAffiliateToast('Ã¢Å“ Bespoke outreach pitch copied to clipboard!');
        }
    };
    ...
    window.shareToTelegram = function(link) {
        const text = encodeURIComponent(`Ã¢Å¡Â¡ Deploy Enterprise AI Workforce Swarms with zero Zapier fees: ${link}`);
    ...
    window.openAddProspectModal = function() {
        const name = prompt('Ã°Å¸¤ Enter Prospect Name (e.g., Tanvir Ahmed):');
        if (!name) return;
        const company = prompt('Ã°Å¸ÂÂ¢ Enter Prospect Company Name:');
        const niche = prompt('Ã°Å¸Å½Â¯ Enter Niche (e.g., E-Commerce, SaaS, Real Estate):', 'E-Commerce');
        const channel = prompt('Ã°Å¸¬ Enter Outreach Channel (e.g., WhatsApp, LinkedIn, Cold Email):', 'WhatsApp');
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã¢Å“ Prospect "${name}" added to your outreach CRM!`);
    };
    ...
    window.sendDirectProspectMessage = function(prospectId) {
        ...
        window.showAffiliateToast(`Ã¢Å“ Direct pitch for ${p.name} copied to clipboard!`);
    };
    ...
    window.copyStudioLink = function() {
        ...
        if (output) {
            ...
            window.showAffiliateToast('Ã¢Å“ Custom Tracking Link copied to clipboard!');
        }
    };
    ...
    window.copyDeepLinkDirect = function(productId, refCode) {
        ...
        window.showAffiliateToast(`Ã¢Å“ Direct Product Link copied for ${productId}!`);
    };
    ...
    window.copyAssetText = function(elemId) {
        ...
        if (el) {
            ...
            window.showAffiliateToast('Ã¢Å“ Marketing Swipe Copy copied to clipboard!');
        }
    };
    ...
    window.generateCustomNicheStrategy = function() {
        const niche = prompt('Ã°Å¸Å½Â¯ Select target niche (e.g. "E-Commerce", "Real Estate", "SaaS", "Clinics"):', 'E-Commerce');
        if (niche) {
            const box = document.getElementById('ai-coach-custom-box');
            if (box) {
                box.innerHTML = `
                    <strong style="color:var(--accent-cyan); display:block; margin-bottom:6px;">Ã°Å¸Å½Â¯ Custom AI Blueprint for ${niche}:</strong>
                    1. <strong>Primary Offer:</strong> 24/7 WhatsApp & Messenger AI Sales Bot ($750 USD / Ã Â§Â³91,875 BDT).<br>
                    2. <strong>Outreach Hook:</strong> Send the Bangla/English WhatsApp swipe file to ${niche} business pages.<br>
                    3. <strong>Your Earning:</strong> 25% Elite Commission = <strong>$187.50 USD (Ã Â§Â³22,968 BDT)</strong> per client closed!<br>
                    4. <strong>Estimated 7-Day Target:</strong> 3 Client Closes = <strong>$562.50 USD (Ã Â§Â³68,906 BDT)</strong>!
                `;
            }
            window.showAffiliateToast(`Ã¢Å“Â¨ 7-Day Custom Strategy generated for ${niche}!`);
        }
    };
    ...
    window.requestPartnerWithdrawal = function() {
        const partner = getActivePartner();
        if (partner.unpaid <= 0) {
            alert('Ã¢Å¡Â Ã¯Â¸Â No unpaid commissions currently available to withdraw.');
            return;
        }

        const confirmed = confirm(
            `Ã¢Å¡Â¡ WITHDRAW COMMISSION DISBURSEMENT\n\n` +
            `Amount: $${partner.unpaid.toFixed(2)} USD (Ã¢† Ã Â§Â³${Math.round(partner.unpaid * 122.50).toLocaleString()} BDT)\n` +
        ...
        if (confirmed) {
            ...
            window.initAffiliatePortal();
            window.showAffiliateToast(`Ã¢Å“ Payout of $${txs[0].amount} initiated to ${partner.payoutAccount}!`);
        }
    };

    window.openPartnerSettingsModal = function() {
        const partner = getActivePartner();
        const name = prompt('Ã°Å¸¤ Enter Full / Agency Name:', partner.name);
        if (!name) return;
        const phone = prompt('Ã°Å¸± Enter WhatsApp / Phone Number:', partner.phone);
        const payoutMethod = prompt('Ã°Å¸³ Enter Payout Method (bKash / Nagad / Wise / Bank / USDT):', partner.payoutMethod);
        const payoutAccount = prompt('Ã°Å¸ÂÂ¦ Enter Payout Account / Wallet Number:', partner.payoutAccount);
        const refCode = prompt('Ã°Å¸ Enter Custom Referral Slug / Code:', partner.refCode);
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast('Ã¢Å“ Partner profile and payment credentials updated successfully!');
    };
        ...
        const modalHtml = `
            <div id="affiliate-qr-modal" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:999999; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(10px);" onclick="if(event.target===this) this.remove()">
                <div style="background:#0b1329; border:1px solid var(--accent-cyan); border-radius:16px; max-width:400px; width:90%; padding:28px; color:#fff; text-align:center; box-shadow:0 0 50px rgba(0,242,254,0.3); font-family:'Inter',sans-serif;">
                    <h3 style="margin:0 0 6px 0; color:var(--accent-cyan); font-size:1.2rem;">Ã°Å¸± Partner QR Code</h3>
