 ...
 *  - 4-Tier Gamified Progression Ladder (Bronze Ã¢Å¾â€ Silver Ã¢Å¾â€ Gold Ã¢Å¾â€ Legend) with Milestone Rewards
...
(function() {
    ...
    const OFFERS = [
        { id: 'b2b-lead-swarm', name: 'B2B SaaS 5-Agent Hunter Swarm', priceUSD: 850, priceBDT: 104125, commUSD: 212.50, commBDT: 26031, epc: '$8.40', conversion: '3.4%', category: 'Lead Gen', tag: 'Ã°Å¸â€Â¥ Top Converting', target: 'store.html' },
        ...
        { id: 'voice-ai-receptionist', name: 'AI Voice Receptionist (Twilio + WebRTC)', priceUSD: 1800, priceBDT: 220500, commUSD: 450.00, commBDT: 55125, epc: '$12.20', conversion: '2.1%', category: 'Voice AI', tag: 'Ã°Å¸Å½â„¢Ã¯Â¸Â High Ticket', target: 'store.html' },
        { id: 'n8n-docker-cluster', name: 'Self-Hosted n8n VPS Cluster Deployment', priceUSD: 497, priceBDT: 60882, commUSD: 124.25, commBDT: 15220, epc: '$6.50', conversion: '5.8%', category: 'Infra', tag: 'Ã°Å¸â€™Â° 90% Cost Saving', target: 'store.html' },
    ...
    const MARKETING_VAULT = [
        {
            id: 'mv-li-01',
            channel: 'Ã°Å¸â€œÂ± LinkedIn / X Viral Post',
        ...
        {
            id: 'mv-email-01',
            channel: 'Ã¢Å“â€°Ã¯Â¸Â CEO / Founder Cold Email Hook',
        ...
        {
            id: 'mv-wa-en-01',
            channel: 'Ã°Å¸â€™Â¬ WhatsApp / SMS Quick DM (English)',
        ...
        root.innerHTML = `
            <div id="affiliate-portal-wrapper" style="display:flex; flex-direction:column; gap:24px;">
                ...
                <div class="ipc-card ipc-card-glowing" style="background:rgba(15,23,42,0.95); border:1px solid var(--accent-cyan); border-radius:18px; padding:24px; box-shadow:0 20px 50px rgba(0,0,0,0.8);">
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:14px; margin-top:20px;">
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(16,185,129,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸â€™Â° Unpaid Balance (Ready)</span>
                            ...
                            <span style="font-size:0.72rem; color:#38bdf8;">Ã¢â€°Ë† Ã Â§Â³${Math.round(partner.unpaid * 122.50).toLocaleString()} BDT</span>
                        </div>
                        ...
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(0,242,254,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸â€™Å½ Lifetime Disbursed</span>
                        ...
                        <div style="background:rgba(0,0,0,0.4); border:1px solid rgba(168,85,247,0.3); border-radius:12px; padding:16px; text-align:center;">
                            <span style="font-size:0.72rem; color:#94a3b8; text-transform:uppercase; font-family:var(--font-mono);">Ã°Å¸â€œË† Average EPC (Yield)</span>
                ...
                <div style="display:flex; gap:8px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:8px; overflow-x:auto; -webkit-overflow-scrolling:touch;">
                    ...
                    <button onclick="window.switchAffiliateTab('payouts')" class="lab-tab" id="tab-btn-payouts" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸â€™Â³ Payout Ledger</button>
                    <button onclick="window.switchAffiliateTab('academy')" class="lab-tab" id="tab-btn-academy" style="padding:8px 16px; border-radius:8px; font-weight:700; font-size:0.85rem; border:1px solid transparent;">Ã°Å¸â€œÅ¡ Growth Academy</button>
                </div>
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(300px, 1fr)); gap:20px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0;">
                            <h3 style="color:#fff; font-size:1.05rem; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                                <span>Ã°Å¸â€ºÂ°Ã¯Â¸Â</span> Real-Time Traffic Telemetry
                            </h3>
                            ...
                            <div style="margin-top:14px; padding:10px; background:rgba(52,211,153,0.1); border:1px solid rgba(52,211,153,0.3); border-radius:8px; font-size:0.78rem; color:#a7f3d0; font-family:var(--font-mono);">
                                Ã°Å¸â€ºÂ¡Ã¯Â¸Â Server-Side Fraud Filter: <strong>${partner.trafficQuality}</strong>
                            </div>
                        </div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0;">
                            <h3 style="color:#fff; font-size:1.05rem; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                                <span>Ã°Å¸â€œÂ²</span> 1-Click Social Broadcast Station
                            </h3>
                ...
                <div id="tab-panel-workspace" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px; border-color:var(--accent-cyan);">
                        ...
                        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:14px; margin-bottom:16px;">
                            ...
                            <div>
                                ...
                                <select id="ai-work-niche" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                    <option value="ecom">Ã°Å¸â€ºâ€™ E-Commerce & F-Commerce Store</option>
                                    <option value="saas">Ã°Å¸â€™Â» B2B SaaS & Tech Agency</option>
                            ...
                            <div>
                                ...
                                <select id="ai-work-format" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                    <option value="wa_bn">Ã°Å¸â€™Â¬ WhatsApp Direct Pitch (Bangla)</option>
                                    <option value="wa_en">Ã°Å¸â€™Â¬ WhatsApp Direct Pitch (English)</option>
                                    <option value="li_post">Ã°Å¸â€œÂ± LinkedIn Viral Post</option>
                                    <option value="cold_email">Ã¢Å“â€°Ã¯Â¸Â B2B Cold Email Sequence</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <button onclick="window.generateCustomMarketingPitch()" class="btn btn-primary" style="width:100%; justify-content:center; padding:12px; font-weight:800; font-size:0.95rem; margin-bottom:16px;">
                            Ã¢Å¡Â¡ Generate Bespoke Marketing Copy & Link Ã¢â€ â€™
                        </button>
                        ...
                        <div style="position:relative;">
                            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                                ...
                                <button onclick="window.copyGeneratedPitch()" class="btn btn-glass-sm" style="font-size:0.72rem; padding:4px 10px;">Ã°Å¸â€œâ€¹ Copy Text</button>
                            </div>
                                    ...
                                    ${getProspects().map(p => `
                                        <tr style="border-bottom:1px solid rgba(255,255,255,0.04); font-family:var(--font-mono);">
                                            ...
                                            <td style="padding:10px;">
                                                <button onclick="window.sendDirectProspectMessage('${p.id}')" class="btn btn-glass-sm" style="font-size:0.7rem; padding:2px 8px;">Ã°Å¸â€™Â¬ Message</button>
                                            </td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                    ...
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px;">
                        ...
                        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:14px; margin-bottom:18px;">
                            <div>
                                ...
                                <select id="link-studio-target" onchange="window.updateStudioGeneratedLink()" style="width:100%; background:rgba(0,0,0,0.5); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px; color:#fff; font-size:0.85rem;">
                                    ...
                                    <option value="/store.html">Ã°Å¸â€ºâ€™ Turnkey Service Store ($29 - $1,800)</option>
                                    <option value="/marketplace.html">Ã°Å¸ÂÂ¬ AI Multi-Agent Marketplace</option>
                                    <option value="/compare.html">Ã¢Å¡â€Ã¯Â¸Â Comparison & ROI Matrix</option>
                                </select>
                            </div>
                        ...
                        <div style="background:rgba(0,0,0,0.6); border:1px solid var(--accent-cyan); border-radius:10px; padding:14px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                            ...
                            <div style="display:flex; gap:8px;">
                                <button onclick="window.copyStudioLink()" class="btn btn-primary-sm" style="font-weight:700;">Ã°Å¸â€œâ€¹ Copy Link</button>
                                <button onclick="window.toggleQrCodeModal()" class="btn btn-glass-sm">Ã°Å¸â€œÂ± QR Code</button>
                            </div>
                        </div>

                        <!-- Deep Link Offer Cards -->
                        <div style="margin-top:28px;">
                            <h4 style="color:#fff; font-size:0.95rem; margin-bottom:12px;">Ã°Å¸â€œÂ¦ Product-Specific Direct Deep Links:</h4>
                            <div style="display:flex; flex-direction:column; gap:8px;">
                                ${OFFERS.map(o => `
                                    <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06); border-radius:8px; padding:10px 14px; flex-wrap:wrap; gap:8px;">
                                        ...
                                        <button onclick="window.copyDeepLinkDirect('${o.id}', '${partner.refCode}')" class="btn btn-glass-sm" style="font-size:0.72rem; padding:4px 10px;">Ã°Å¸â€œâ€¹ Copy Product Link</button>
                                    </div>
                                `).join('')}
                            </div>
                        </div>
                ...
                <div id="tab-panel-vault" class="aff-tab-panel" style="display:none;">
                    ...
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(300px, 1fr)); gap:16px;" id="vault-assets-grid">
                        ${MARKETING_VAULT.map(a => {
                            const populated = a.text.replace(/\{\{REF_LINK\}\}/g, partnerRefLink).replace(/\{\{PartnerName\}\}/g, partner.name).replace(/\{\{CompanyName\}\}/g, 'Prospect Inc').replace(/\{\{FirstName\}\}/g, 'Founder').replace(/\{\{Name\}\}/g, 'Ã Â¦Â­Ã Â¦Â¾Ã Â¦â€¡');
                            return `
                                <div class="ipc-card vault-item" data-cat="${a.category}" style="background:rgba(15,23,42,0.85); margin:0; padding:18px;">
                                    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;">
                                        ...
                                        <button onclick="window.copyAssetText('${a.id}')" class="btn btn-primary-sm" style="padding:4px 10px; font-size:0.72rem;">Ã°Å¸â€œâ€¹ Copy</button>
                                    </div>
                ...
                <div id="tab-panel-coach" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:24px; border-radius:16px; border-color:var(--accent-cyan);">
                        ...
                        <div id="ai-coach-custom-box" style="background:rgba(0,0,0,0.6); border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:16px; font-size:0.82rem; color:#cbd5e1; line-height:1.6;">
                            <strong style="color:var(--accent-cyan); display:block; margin-bottom:4px;">Ã°Å¸â€™Â¡ AI Partner Recommendation:</strong>
                ...
                <div id="tab-panel-milestones" class="aff-tab-panel" style="display:none;">
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:var(--accent-cyan); box-shadow:0 0 20px rgba(0,242,254,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸â€Â¥ VIP Silver</div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:var(--accent-gold); box-shadow:0 0 20px rgba(245,158,11,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸â€˜â€˜ Elite Gold (Active)</div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); border-color:#ec4899; box-shadow:0 0 20px rgba(236,72,153,0.2); margin:0;">
                            <div style="font-size:1.4rem;">Ã°Å¸â€™Å½ Legend Diamond</div>
                ...
                <div id="tab-panel-payouts" class="aff-tab-panel" style="display:none;">
                    <div class="ipc-card" style="background:rgba(15,23,42,0.9); padding:22px; border-radius:16px; margin:0;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:10px;">
                            <h3 style="color:#fff; font-size:1.15rem; margin:0;">Ã°Å¸â€™Â³ Canonical Transaction & Payout Ledger</h3>
                ...
                <div id="tab-panel-academy" class="aff-tab-panel" style="display:none;">
                    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:16px;">
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0; padding:20px;">
                            <span style="font-size:1.8rem;">Ã°Å¸Å½â€œ</span>
                            ...
                            <button onclick="alert('Ã°Å¸â€œËœ Module 1 Guide:\n\n1. Search founders hiring for Operations/Data entry on LinkedIn.\n2. Send Swipe #1.\n3. When they reply, drop your affiliate link to book the free video teardown.')" class="btn btn-glass-sm" style="width:100%; margin-top:10px;">Read Playbook Ã¢â€ â€™</button>
                        </div>
                        ...
                        <div class="ipc-card" style="background:rgba(15,23,42,0.85); margin:0; padding:20px;">
                            ...
                            <button onclick="alert('Ã°Å¸â€œËœ Module 2 Guide:\n\n1. Target e-commerce brands with 50+ daily orders.\n2. Send Bangla WhatsApp swipe.\n3. Show how the bot answers in 3 seconds & confirms cash-on-delivery orders.')" class="btn btn-glass-sm" style="width:100%; margin-top:10px;">Read Playbook Ã¢â€ â€™</button>
                        </div>
        ...
        root.innerHTML = `
            <div id="affiliate-auth-gateway" class="ipc-card ipc-card-glowing" style="background:rgba(15,23,42,0.95); border:1px solid var(--accent-cyan); border-radius:18px; padding:32px; max-width:850px; margin:0 auto; box-shadow:0 25px 60px rgba(0,0,0,0.85);">
                ...
                <div style="display:flex; justify-content:center; gap:10px; margin-bottom:24px;">
                    <button onclick="window.switchAuthMode('signin')" id="auth-tab-signin" class="lab-tab active" style="padding:10px 24px; font-size:0.9rem; font-weight:800; border-radius:10px;">Ã°Å¸â€â€˜ Partner Sign In</button>
                    <button onclick="window.switchAuthMode('register')" id="auth-tab-register" class="lab-tab" style="padding:10px 24px; font-size:0.9rem; font-weight:800; border-radius:10px;">Ã°Å¸Å¡â‚¬ Register New Account</button>
                </div>
                    ...
                    <form onsubmit="window.handlePartnerRegister(event)" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:16px;">
                        ...
                        <div>
                            ...
                            <select id="reg-payout-method" required style="width:100%; background:rgba(15,23,42,0.9); border:1px solid rgba(255,255,255,0.2); border-radius:8px; padding:10px 14px; color:#fff; font-size:0.85rem; box-sizing:border-box;">
                                <option value="bKash (Personal / Merchant)">Ã°Å¸â€¡Â§Ã°Å¸â€¡Â© bKash (Personal / Merchant)</option>
                                <option value="Nagad">Ã°Å¸â€¡Â§Ã°Å¸â€¡Â© Nagad</option>
                        ...
                        <div style="grid-column: 1 / -1; margin-top:8px;">
                            <button type="submit" class="btn btn-primary" style="width:100%; background:linear-gradient(135deg, #10b981, #059669); padding:14px; font-size:1rem; font-weight:800; border-radius:10px; cursor:pointer; justify-content:center;">
                                Ã°Å¸Å¡â‚¬ Create Account & Launch Dashboard Ã¢â€ â€™
                            </button>
                        </div>
                    </form>
                </div>
    ...
    window.handlePartnerSignIn = function(e) {
        ...
        if (matchedPartner) {
            ...
            window.showAffiliateToast(`Ã°Å¸â€˜â€¹ Welcome back, ${matchedPartner.name}!`);
        } else {
            ...
            window.initAffiliatePortal();
            window.showAffiliateToast(`Ã°Å¸Å½â€° Welcome to IINSHA Partner OS, ${newP.name}!`);
        }
    };
    ...
    window.handlePartnerRegister = function(e) {
        ...
        setActivePartner(newId);
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã°Å¸Å½â€° Account Created! Welcome, ${name}! Your link: /?ref=${newId}`);
    };
    ...
    window.quickDemoLogin = function(partnerId) {
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã°Å¸â€˜â€˜ Switched account to ${partnerId}!`);
    };
    ...
    window.copyGeneratedPitch = function() {
        ...
        if (outputEl) {
            ...
            window.showAffiliateToast('Ã¢Å“â€¦ Bespoke outreach pitch copied to clipboard!');
        }
    };
    ...
    window.openAddProspectModal = function() {
        const name = prompt('Ã°Å¸â€˜Â¤ Enter Prospect Name (e.g., Tanvir Ahmed):');
        if (!name) return;
        ...
        const channel = prompt('Ã°Å¸â€™Â¬ Enter Outreach Channel (e.g., WhatsApp, LinkedIn, Cold Email):', 'WhatsApp');
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast(`Ã¢Å“â€¦ Prospect "${name}" added to your outreach CRM!`);
    };
    ...
    window.sendDirectProspectMessage = function(prospectId) {
        ...
        window.showAffiliateToast(`Ã¢Å“â€¦ Direct pitch for ${p.name} copied to clipboard!`);
    };
    ...
    window.copyStudioLink = function() {
        ...
        if (output) {
            ...
            window.showAffiliateToast('Ã¢Å“â€¦ Custom Tracking Link copied to clipboard!');
        }
    };
    ...
    window.copyDeepLinkDirect = function(productId, refCode) {
        ...
        window.showAffiliateToast(`Ã¢Å“â€¦ Direct Product Link copied for ${productId}!`);
    };
    ...
    window.copyAssetText = function(elemId) {
        ...
        if (el) {
            ...
            window.showAffiliateToast('Ã¢Å“â€¦ Marketing Swipe Copy copied to clipboard!');
        }
    };
    ...
    window.requestPartnerWithdrawal = function() {
        ...
        const confirmed = confirm(
            ...
            `Amount: $${partner.unpaid.toFixed(2)} USD (Ã¢â€°Ë† Ã Â§Â³${Math.round(partner.unpaid * 122.50).toLocaleString()} BDT)\n` +
        ...
        if (confirmed) {
            ...
            window.initAffiliatePortal();
            window.showAffiliateToast(`Ã¢Å“â€¦ Payout of $${txs[0].amount} initiated to ${partner.payoutAccount}!`);
        }
    };

    window.openPartnerSettingsModal = function() {
        const partner = getActivePartner();
        const name = prompt('Ã°Å¸â€˜Â¤ Enter Full / Agency Name:', partner.name);
        if (!name) return;
        ...
        window.initAffiliatePortal();
        window.showAffiliateToast('Ã¢Å“â€¦ Partner profile and payment credentials updated successfully!');
    };
        ...
        const modalHtml = `
            <div id="affiliate-qr-modal" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:999999; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(10px);" onclick="if(event.target===this) this.remove()">
                <div style="background:#0b1329; border:1px solid var(--accent-cyan); border-radius:16px; max-width:400px; width:90%; padding:28px; color:#fff; text-align:center; box-shadow:0 0 50px rgba(0,242,254,0.3); font-family:'Inter',sans-serif;">
                    <h3 style="margin:0 0 6px 0; color:var(--accent-cyan); font-size:1.2rem;">Ã°Å¸â€œÂ± Partner QR Code</h3>
