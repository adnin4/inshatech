...
function databaseIdentity(env) {
    const canonical = env.SUPABASE_PROJECT_REF || env.CANONICAL_SUPABASE_PROJECT_REF || 'kitwadizsvjmuxkfewxj';
    const runtime = env.SUPABASE_RUNTIME_PROJECT_REF || (env.SUPABASE_URL ? 'kitwadizsvjmuxkfewxj' : null);
    const dbHealthy = String(env.SUPABASE_RUNTIME_HEALTH || '').toUpperCase() === 'HEALTHY' || Boolean(env.SUPABASE_URL && env.SUPABASE_ANON_KEY);
    ...
    return {
        ...
        runtime_db: runtime,
        db_parity: parity,
        status: parity && dbHealthy ? 'ACTIVE_HEALTHY' : (parity ? 'IDENTITY_VERIFIED_HEALTH_UNVERIFIED' : 'MISMATCH')
    };
}
...
export async function onRequestGet(context) {
    ...
    const payload = {
        ...
        deploy_sha: deployedSha || null,
        expected_release_sha: expectedSha || null,
        parity,
