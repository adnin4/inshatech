...
async function runTests() {
    ...
    // TEST 1: Rejection of unknown service_id
    {
        ...
        assert(res.status === 400 && json.code === 'INVALID_SERVICE_ID', 'Rejects unknown service_id with HTTP 400');
    }
    ...
    // TEST 3: Server Catalog Authority Overrides Client-Manipulated Amount
    {
        const req = new Request('https://inshatech.pages.dev/api/payments/checkout', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        assert(
            ...
            'Server catalog strictly enforces authoritative price (), rejecting client-side  attempt'
        );
    }
    ...
    // TEST 7: Read-Only Browser Return Endpoint Rejects POST with 405
    {
    ...
    // TEST 8: Scoped Middleware Intercepts GET /api/payments/webhook -> Redirects to /api/payments/return
    {
