/* Global Observability & Diagnostic Guard */
window.onerror = function(msg, url, lineNo, columnNo, error) {
    ...
    if (typeof console !== 'undefined' && console.error) {
        console.error("IINSHA Telemetry Error Logged:", errorDetails);
    }
    // Record to diagnostic buffer if available
    if (window.__iinsha_error_log && Array.isArray(window.__iinsha_error_log)) {
        window.__iinsha_error_log.push(errorDetails);
    } else {
        window.__iinsha_error_log = [errorDetails];
    }
    return false; // Allow standard error diagnostics and testing harnesses to capture errors
};
...
/* getAIExecutiveBriefing primary definition is established above */
    ...
    container.innerHTML = `
        <div style="background: rgba(3, 7, 18, 0.98); border: 1px solid var(--accent-gold); border-radius: 16px; padding: 24px; color: #fff; box-shadow: 0 20px 50px rgba(0,0,0,0.8);">
            ...
            <div style="display:flex; gap:6px; overflow-x:auto; padding-bottom:10px; margin-bottom:20px; border-bottom:1px solid rgba(255,255,255,0.08);">
                ...
                <button onclick="ibosSwitchTab('market')" class="btn btn-glass-sm ${ibosActiveTab === 'market' ? 'active-ibos-tab' : ''}" style="${ibosActiveTab === 'market' ? 'background:var(--accent-cyan); color:#000; font-weight:800;' : ''}">ðŸ› Marketplace 2.0</button>
                    ...
                    ${services.map((svc, sIdx) => `
                        <div style="background:rgba(15,23,42,0.8); padding:14px; border-radius:10px; border:1px solid rgba(255,255,255,0.08); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                            ...
                            <div style="display:flex; gap:8px;">
                                <button class="btn btn-glass-sm ibos-btn-edit-svc" data-svc-idx="${sIdx}" style="font-size:0.75rem; color:var(--accent-cyan); border-color:var(--accent-cyan);">âœ Edit Service</button>
                                <button class="btn btn-glass-sm ibos-btn-delete-svc" data-svc-idx="${sIdx}" style="font-size:0.75rem; color:#fca5a5; border-color:#ef4444;">ðŸ—‘ Delete</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">
                    ${adminUsers.map((adm, aIdx) => `
                        <div style="background:rgba(15,23,42,0.8); padding:14px; border-radius:10px; border:1px solid rgba(255,255,255,0.08); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                            ...
                            <button class="btn btn-glass-sm ibos-btn-edit-admin" data-admin-idx="${aIdx}" style="font-size:0.75rem; color:var(--accent-gold); border-color:var(--accent-gold);">âœ Change Credentials</button>
                        </div>
                    `).join('')}
                </div>
            </div>
...
function bindIBOSModuleListeners() {
    ...
    if (btnUpdateRate) {
        btnUpdateRate.addEventListener('click', () => {
            ...
            if (isNaN(inputRate) || inputRate <= 0) {
                alert('âš  Please enter a valid exchange rate (e.g. 120)');
                return;
            }
            const words = getSiteWords();
    ...
    document.querySelectorAll('.ibos-btn-delete-svc').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (confirm(`âš  Are you sure you want to delete service "${svc.title}"?`)) {
                registry.splice(idx, 1);
                saveServiceRegistry(registry);
                alert(`ðŸ—‘ Service deleted from OS Registry.`);
                renderAdminModalCmsStudio(document.getElementById('index-admin-cms-root'));
            }
        });
    });
    ...
    container.innerHTML = `
        <div id="admin-login-card" style="max-width: 500px; margin: 20px auto; padding: 32px; background: rgba(15, 23, 42, 0.98); border: 1px solid var(--accent-gold); border-radius: 16px; backdrop-filter: blur(16px); box-shadow: 0 25px 60px rgba(0, 0, 0, 0.95); color: #fff; text-align: center;">
            ...
            <form id="admin-modal-login-form" onsubmit="handleAdminLoginSubmit(event)" style="text-align: left;">
                ...
                <div id="admin-login-error" style="color: #ef4444; font-size: 0.8rem; margin-bottom: 12px; display: none;">âš  Passphrase must be at least 6 characters.</div>

                <div style="display: flex; gap: 10px; margin-bottom: 12px;">
                    <button type="submit" class="btn btn-primary" style="flex: 1; padding: 12px; background: linear-gradient(135deg, var(--accent-gold), #d97706); color: #000; font-weight: 800; font-size: 0.95rem; border: none; border-radius: 8px; cursor: pointer;">ðŸ”“ Authenticate & Open Studio →</button>
                </div>
            </form>
            ...
            <div style="margin-top: 14px; font-family: var(--font-mono); font-size: 0.7rem; color: var(--text-dim); border-top: 1px solid var(--glass-border); padding-top: 10px;">
                🔒 Single Source Kernel â— 15 Domains â— RBAC Active
            </div>
        </div>
    `;
}
...
function handleAdminLoginSubmit(e) {
    ...
    if (email && pass && pass.length >= 6) {
        ...
        if (errEl) {
            ...
            errEl.textContent = 'âš  Invalid credentials. Passphrase must be at least 6 characters.';
        }
    }
}
    ...
    if (!document.getElementById('floating-chat-mic-btn')) {
        ...
        micBtn.addEventListener('click', () => {
            ...
            if (!SpeechRecognition) {
                alert('âš  Voice recognition is not supported in your current browser. Please use Chrome, Edge, or Brave.');
                return;
            }
...
function initI18nLanguageSwitcher() {
    ...
    const translations = {
        bn: {
            ...
            btnControl: '🎛️ à¦®à¦¾à¦¸à§à¦Ÿà¦¾à¦° à¦•à¦¨à§à¦Ÿà§à¦°à§‹à¦² à¦ªà§à¦¯à¦¾à¦¨à§‡à¦²'
        },
        en: {
    ...
    window.toggleSiteLanguage = function() {
        ...
        if (btnLang) btnLang.textContent = lang === 'en' ? '🇧🇩 à¦¬à¦¾à¦‚à¦²à¦¾' : 'ðŸ‡¬ðŸ‡§ English';
...
function initAiVoiceSimulator() {
    ...
    startCallBtn.onclick = () => {
        ...
        if (isCalling) {
            ...
            startCallBtn.innerHTML = 'ðŸŽ™ Simulate AI Telephony Voice Call';
            ...
            html += `
                <div class="glass-card glowing-border" style="padding:20px; border-radius:14px; background:rgba(15,23,42,0.8); border:1px solid rgba(255,255,255,0.1);">
                    ...
                    <div style="display:flex; justify-content:space-between; font-size:0.75rem; color:#94a3b8; border-top:1px solid rgba(255,255,255,0.08); padding-top:8px;">
                        <span>â± Delivery: ${s.deliveryTime}</span>
        ...
        rowsHtml += `
            <tr style="border-bottom:1px solid rgba(255,255,255,0.08);">
                ...
                <td style="padding:10px;">
                    <button onclick="editServiceInline('${s.id}')" class="btn btn-glass-sm" style="padding:4px 8px; font-size:0.75rem; margin-right:4px;">âœ Edit</button>
                    <button onclick="deleteServiceAction('${s.id}')" class="btn btn-danger-sm" style="padding:4px 8px; font-size:0.75rem;">ðŸ—‘ Delete</button>
                </td>
            </tr>
        `;
    });
    ...
    crudContainer.innerHTML = `
        <div class="glass-card glowing-border" style="padding:20px; background:rgba(3,7,18,0.95); border:1px solid var(--accent-cyan); border-radius:14px; margin-top:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
                <div>
                    <h3 style="margin:0; color:#fff; font-size:1.1rem;">ðŸ›  FULL CRUD SERVICE & MARKETPLACE MANAGER</h3>
...
function deleteServiceAction(id) {
    ...
    initSecurityAuditTerminal();
    initLanguageSwitcher();
        alert("ðŸ—‘ Service Deleted Successfully!");
    }
}
...
function initLanguageSwitcher() {
    ...
    langBtn.onclick = () => {
        ...
        alert(currentLang === 'BN' ? '🇧🇩 à¦¬à¦¾à¦‚à¦²à¦¾ à¦­à¦¾à¦·à¦¾ à¦®à§‹à¦¡ à¦¸à¦•à§à¦°à¦¿à§Ÿ à¦•à¦°à¦¾ à¦¹à§Ÿà§‡à¦›à§‡!' : '🇺🇸 Switched to English Language Mode!');
    };
}
            ...
            <div style="margin-bottom:12px; text-align:${isAi ? 'left' : 'right'};">
                <div style="display:inline-flex; align-items:center; gap:6px; margin-bottom:3px; float:${isAi ? 'none' : 'right'};">
                    <span style="font-size:0.75rem; color:${isAi ? 'var(--accent-cyan)' : 'var(--accent-emerald)'}; font-weight:bold;">
                        ${isAi ? '🤖 IINSHA Enterprise AI' : 'ðŸ‘¤ You (Client)'}
                    </span>
                </div>
...
/* duplicate */ var initFloatingAiAssistantWidget_alias = function() {
    const old = document.getElementById('iinsha-floating-ai-widget');
    if (old) old.remove();
...
/* duplicate */ var initMasterApp_alias = function() {
    try { if (typeof initGlobalCurrencyConverter === 'function') initGlobalCurrencyConverter(); } catch(e){}
    try { if (typeof initInteractiveDiagramVisualizer === 'function') initInteractiveDiagramVisualizer(); } catch(e){}
    try { if (typeof initRoiComparisonMatrix === 'function') initRoiComparisonMatrix(); } catch(e){}
    try { if (typeof initInteractiveAiAgentBuilder === 'function') initInteractiveAiAgentBuilder(); } catch(e){}
...
/* duplicate */ var openAiOrderConsultationModal_alias = function(serviceName = 'AI Automation Solution', packageTier = 'Professional Tier', price = 499) {
    currentAiOrderState.serviceName = serviceName;
    currentAiOrderState.packageTier = packageTier;
    currentAiOrderState.price = price;
    currentAiOrderState.discountedPrice = Math.round(price * 0.9);
...
/* duplicate */ var closeAiOrderConsultationModal_alias = function() {
    const modal = document.getElementById('ai-order-consultation-modal');
    if (modal) {
        modal.classList.remove('active');
        modal.style.display = 'none';
...
/* duplicate */ var renderAiOrderModalContent_alias = function() {
    const modal = document.getElementById('ai-order-consultation-modal');
    if (!modal) return;
...
/* duplicate */ var sendQuickChipToAi_alias = function(text) {
    const input = document.getElementById('ai-chat-input-text');
    if (input) input.value = text;
    sendUserMessageToAiOrderAgent();
}
...
/* duplicate */ var sendUserMessageToAiOrderAgent_alias = function() {
    const input = document.getElementById('ai-chat-input-text');
    if (!input || !input.value.trim()) return;
...
/* duplicate */ var confirmOrderAndSyncToWhatsApp_alias = function() {
    const nameInput = document.getElementById('order-client-name');
    const phoneInput = document.getElementById('order-client-phone');
    const emailInput = document.getElementById('order-client-email');
            ...
            <div style="flex:1; overflow-y:auto; padding-right:6px;">
                ...
                ` : `
                    ...
                    <div style="display:grid; gap:12px;">
                        ...
                        <div style="background:rgba(30,41,59,0.6); border:1px solid rgba(255,255,255,0.1); padding:14px; border-radius:12px;">
                            <div style="font-size:0.85rem; font-weight:bold; color:var(--accent-emerald); margin-bottom:6px;">âœ‰ Cold Outreach Email Script</div>
        ...
        <div style="margin-top:40px; background:rgba(15,23,42,0.8); border:1px solid var(--accent-cyan); padding:32px 20px; border-radius:20px; box-shadow:0 0 35px rgba(6,182,212,0.15);">
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap:18px;">
                ...
                <div class="glass-card glowing-border" style="padding:20px; background:rgba(30,41,59,0.8); border:1px solid rgba(16,185,129,0.3); border-radius:14px;">
                    ...
                    <button onclick="openClientAffiliatePortalModal('affiliate')" class="btn btn-emerald-sm" style="width:100%; font-weight:bold;">ðŸ¤ Open Partner Dashboard →</button>
                </div>
    ...
    root.innerHTML = `
        <div class="glass-card glowing-border" style="background:rgba(15,23,42,0.9); border:1px solid var(--accent-emerald); border-radius:20px; padding:32px 24px; max-width:850px; margin:40px auto; box-shadow:0 0 40px rgba(16,185,129,0.2);">
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap:20px; margin-bottom:24px;">
                ...
                <div>
                    <label style="color:#fff; font-size:0.85rem; font-weight:bold; display:flex; justify-content:space-between; margin-bottom:6px;">
                        <span>â± Hours Saved / Week per Person:</span>
...
function toggleAgentKillSwitch(agentId) {
    if (agentId === 'all') {
        ...
        alert(`âš  Agent ${agentId} status updated: Manual Override toggled.`);
    }
...
function togglePeaceMode() {
    ...
    if (btn) {
        btn.innerHTML = iinshaGodModeState.peaceMode ? 'â˜® PEACE MODE ACTIVE (100% Hands-Off)' : 'â˜® Enable 1-Click Peace Mode';
        btn.style.background = iinshaGodModeState.peaceMode ? 'rgba(16,185,129,0.3)' : 'rgba(255,255,255,0.1)';
    }
    alert(iinshaGodModeState.peaceMode ? 'â˜® Peace Mode Activated! The 13 AI Swarm Agents will handle 100% of operations automatically.' : 'Peace Mode Deactivated.');
}
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:14px; margin-bottom:18px;">
                <div>
                    <h4 style="margin:0; color:#fff; font-size:1.25rem; display:flex; align-items:center; gap:10px;">
                        <span>ðŸŒŒ GOD MODE CONTROL CENTER — THE FINAL FORM</span>
...
var iinshaTranslations = {
    ...
    BN: {
        ...
        auditBtn: "⚡ à¦«à§à¦°à¦¿ à¦à¦†à¦‡ à¦¬à¦¿à¦œà¦¨à§‡à¦¸ à¦…à¦¡à¦¿à¦Ÿ à¦•à¦°à§à¦¨",
...
function triggerMasterKillSwitch(mode) {
    ...
    if (mode === 'PAUSE_ALL') {
        ...
        showFomoToast('ðŸ”´ EMERGENCY KILL SWITCH ACTIVATED — ALL 27 AGENTS PAUSED BY OWNER', 'error');
        ...
        showFomoToast('ðŸŸ¡ HIGH-RISK ACTIONS FROZEN — HIGH-RISK MUTATIONS REQUIRING HUMAN GATE', 'warning');
        ...
        showFomoToast('ðŸŸ¢ ALL SYSTEMS RESUMED — SUPREME OWNER GOVERNANCE ACTIVE', 'success');
    }
}
...
var iinshaCatalogV4Matrix = [
    ...
    },
    {
        ...
        services: [
            ...
            { id: 't4_rag_brain', name: 'Enterprise RAG Knowledge Base & Vector Brain', desc: 'Supabase pgvector database storing company knowledge, client docs & tech specs.', priceUSD: '$2,500 setup', marketVal: '$8,000/mo Enterprise AI Brain', roi: 'Full Private Data Sovereignty', bdt: '৳305,000 setup' },
...
var iinshaInternationalServiceCatalog = [
    {
        ...
        services: [
            ...
            { id: 'cat1_rag_agent', name: 'AI Knowledge Base / RAG Agent', desc: 'PDF, doc, and database ingestion with accurate vector retrieval and verified citations.', priceUSD: '$2,500', marketVal: '$7,500/mo Enterprise AI', roi: 'Full Private Data Sovereignty', bdt: '৳3,05,000' },
    ...
    root.innerHTML = `
        <div style="background:linear-gradient(135deg, rgba(15,23,42,0.98), rgba(30,27,75,0.95)); border:1px solid #3b82f6; border-radius:20px; padding:24px; margin-top:20px; box-shadow:0 0 60px rgba(59,130,246,0.35);">
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:14px; margin-bottom:18px;">
                <div>
                    <h4 style="margin:0; color:#fff; font-size:1.25rem; display:flex; align-items:center; gap:10px;">
                        <span>ðŸ‘‘ IINSHA AI OS — 5-DIVISION ENTERPRISE STUDIO</span>
...
/* duplicate */ var openIinshaBusinessAuditModal_alias = function() {
    let auditModal = document.getElementById('iinsha-audit-modal-root');
    if (!auditModal) {
        auditModal = document.createElement('div');
        auditModal.id = 'iinsha-audit-modal-root';
...
/* duplicate */ var closeIinshaBusinessAuditModal_alias = function() {
    const modal = document.getElementById('iinsha-audit-modal-root');
    if (modal) {
        modal.style.display = 'none';
        modal.innerHTML = '';
...
/* duplicate */ var initSolutionFinder_alias = function() {
    const finderInput = document.getElementById('finder-input');
    const finderBtn = document.getElementById('run-finder-btn');
    const outputBox = document.getElementById('finder-output-box');
...
/* duplicate */ var initPresalesAssistant_alias = function() {
    const presalesInput = document.getElementById('presales-query-input');
    const presalesBtn = document.getElementById('presales-submit-btn');
    const presalesOutput = document.getElementById('presales-output-box');
...
/* duplicate */ var initRoiCalculatorEngine_alias = function() {
    ...
    1: {
        ...
        deliverables: `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="glass-card" style="padding: 16px; border-color: rgba(0, 242, 254, 0.3);">
                    ...
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li>Multi-channel lead ingestion (LinkedIn, Apollo, Forms)</li>
                        <li>Real-time qualification scoring via LLM mesh</li>
                        <li>Automated CRM synchronization (HubSpot, Notion)</li>
                        <li>Instant hot-lead routing to WhatsApp & Telegram</li>
                    </ul>
                </div>
                <div class="glass-card" style="padding: 16px; border-color: rgba(16, 185, 129, 0.3);">
                    <strong style="color: var(--accent-emerald); font-size: 0.95rem; display: block; margin-bottom: 6px;">📈 Verified Impact</strong>
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li><strong>Response Time:</strong> &lt; 45 seconds from lead submission</li>
                        <li><strong>Conversion Rate:</strong> 3.2x higher meeting booking rate</li>
                        <li><strong>Labor Saved:</strong> 25+ hours/week of manual SDR work</li>
                    </ul>
                </div>
            </div>
            <div style="background: rgba(0,0,0,0.4); padding: 14px; border-radius: 8px; border-left: 3px solid var(--accent-cyan); font-size: 0.82rem; color: #94a3b8;">
                <strong>Architecture:</strong> Playwright Scraper + n8n Cluster + Supabase PGVector + Resend API.
            </div>
        `
    },
    2: {
        badge: "⚡ OUTCOME 02 — WORKFLOW & PROCESS AUTOMATION",
        title: "Scale: Enterprise n8n Cluster & Webhook Mesh",
        subtitle: "Eliminate repetitive manual operations with self-healing Dockerized n8n workflows and multi-app sync.",
        deliverables: `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="glass-card" style="padding: 16px; border-color: rgba(168, 85, 247, 0.3);">
                    <strong style="color: var(--accent-purple); font-size: 0.95rem; display: block; margin-bottom: 6px;">⚡ Multi-App Sync Mesh</strong>
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li>Zero-Zapier fee self-hosted n8n cluster</li>
                        <li>Bidirectional webhook routing with retry queues</li>
                        <li>Automated invoice OCR & accounting sync</li>
                        <li>Slack & Telegram alert dispatchers</li>
                    </ul>
                </div>
                <div class="glass-card" style="padding: 16px; border-color: rgba(0, 242, 254, 0.3);">
                    <strong style="color: var(--accent-cyan); font-size: 0.95rem; display: block; margin-bottom: 6px;">📈 Cost Optimization</strong>
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li><strong>Monthly Savings:</strong> $300-$1,200/mo vs Zapier/Make</li>
                        <li><strong>Throughput:</strong> 100k+ tasks/mo at flat VPS cost</li>
                        <li><strong>Availability:</strong> 99.9% uptime with Docker failover</li>
                    </ul>
                </div>
            </div>
            <div style="background: rgba(0,0,0,0.4); padding: 14px; border-radius: 8px; border-left: 3px solid var(--accent-purple); font-size: 0.82rem; color: #94a3b8;">
                <strong>Architecture:</strong> Docker Compose + PostgreSQL + n8n Enterprise + Nginx Reverse Proxy.
            </div>
        `
    },
    3: {
        badge: "💬 OUTCOME 03 — 24/7 MULTI-CHANNEL AI AGENTS",
        title: "Engage: WhatsApp, Web & Voice AI Concierge",
        subtitle: "Provide instant customer support, answer product questions from your knowledge base, and book appointments automatically.",
        deliverables: `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="glass-card" style="padding: 16px; border-color: rgba(245, 158, 11, 0.3);">
                    <strong style="color: var(--accent-gold); font-size: 0.95rem; display: block; margin-bottom: 6px;">💬 Omnichannel Concierge</strong>
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li>Official WhatsApp Cloud API integration</li>
                        <li>Interactive website chat widget with RAG</li>
                        <li>Bangla, Banglish & English fluency</li>
                        <li>Human escalation & ticket handover</li>
                    </ul>
                </div>
                <div class="glass-card" style="padding: 16px; border-color: rgba(16, 185, 129, 0.3);">
                    <strong style="color: var(--accent-emerald); font-size: 0.95rem; display: block; margin-bottom: 6px;">📈 Engagement Metrics</strong>
                    <ul style="font-size: 0.82rem; color: #cbd5e1; padding-left: 16px; line-height: 1.6;">
                        <li><strong>Resolution Rate:</strong> 80%+ tier-1 support queries answered</li>
                        <li><strong>Response Latency:</strong> &lt; 2 seconds round-trip</li>
                        <li><strong>Satisfaction:</strong> 4.8/5 CSAT rating</li>
                    </ul>
                </div>
            </div>
            <div style="background: rgba(0,0,0,0.4); padding: 14px; border-radius: 8px; border-left: 3px solid var(--accent-gold); font-size: 0.82rem; color: #94a3b8;">
                <strong>Architecture:</strong> Meta Cloud API + Pinecone Vector DB + Gemini Pro + n8n Webhook.
            </div>
        `
    },
    4: {
...
function openOutcomeDetailModal(outcomeIndex) {
    ...
    const badgeEl = document.getElementById('oim-badge');
    const titleEl = document.getElementById('oim-title');
    const subEl = document.getElementById('oim-subtitle');
    const boxEl = document.getElementById('oim-content-box');
    const waLink = document.getElementById('oim-wa-link');

    if (badgeEl) badgeEl.textContent = data.badge;
    if (titleEl) titleEl.textContent = data.title;
    if (subEl) subEl.textContent = data.subtitle;
    if (boxEl) boxEl.innerHTML = data.deliverables;
...
function openCheckoutModal(packageName, setupPrice, retainerPrice) {
    ...
    const pkgTitle = document.getElementById('checkout-pkg-title');
    const setupVal = document.getElementById('checkout-setup-val');
    const bdtVal = document.getElementById('checkout-bdt-val');

    if (pkgTitle) pkgTitle.textContent = packageName;
    const numPrice = Number(setupPrice) || 100;
    if (setupVal) setupVal.textContent = '$' + numPrice.toLocaleString() + ' USD';
    if (bdtVal) bdtVal.textContent = '৳' + Math.round(numPrice * 122.50).toLocaleString() + ' BDT';
...
async function handleCheckoutFormSubmit(e) {
    ...
    const pkgEl = document.getElementById('checkout-pkg-title');
    const pkg = pkgEl ? pkgEl.textContent : 'AI Automation Package';
    
    const priceEl = document.getElementById('checkout-setup-val');
    const priceTxt = priceEl ? priceEl.textContent : '100';
    ...
    try {
        ...
        if (data.redirect_url) {
            ...
            return;
        } else if (data.gateway && data.gateway.checkout_url) {
